...
describe("products routes", () => {
    ...
    it("rejects duplicate EANs", async () => {
        const admin = await loginAs(testApp.app, "admin@painel.com", "123456");
    ...
    it("rejects invalid product payloads", async () => {
        ...
        const invalidPriceResponse = await request(testApp.app)
            .post("/api/products")
            .set("Authorization", bearer(admin.accessToken))
            .send({
                ean: "7891000100105",
        ...
        expect(invalidPriceResponse.statusCode).toBe(400);

        const invalidStockRangeResponse = await request(testApp.app)
            .post("/api/products")
            .set("Authorization", bearer(admin.accessToken))
            .send({
                ean: "7891000100106",
        ...
        expect(invalidStockRangeResponse.statusCode).toBe(400);
    });

    it("returns not found for an unknown EAN", async () => {
        const vendor = await loginAs(testApp.app, "joao@painel.com", "123456");

        const response = await request(testApp.app)
            .get("/api/products/by-ean/7891000199999")
            .set("Authorization", bearer(vendor.accessToken));
