...
export interface ProductsService {
  ...
  update(id: string, input: UpdateProductBody): UpdateProductResponse | undefined;
  deactivate(id: string): DeactivateProductResponse | undefined;
}

type ProductsStoreItem = ProductDetailResponse;

function deriveIsCritical(item: Pick<ProductsStoreItem, "stockCurrent" | "minStock">): boolean {
...
export function createProductsService(): ProductsService {
  ...
  >({
    ...
    filterItem: (item, search) => {
      ...
      return item.name.toLowerCase().includes(search) || item.ean.includes(search);
    },
    toListItem,
