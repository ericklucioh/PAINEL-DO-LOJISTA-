...
function cloneAuthLoginMock(user: AuthUser, refreshToken: string, accessToken: string): AuthLoginResponse {
  return {
    ...authLoginMock,
    accessToken,
    refreshToken,
...
export function createAuthService({
  ...
  return {
    async login(input) {
      ...
      const passwordMatches = await compare(input.password, account.passwordHash);
      if (!passwordMatches) {
        throw createHttpError("Credenciais inválidas", 401);
      }
      ...
      return cloneAuthLoginMock(toAuthUser(account), refreshToken, accessToken);
    },
    ...
    async refresh(refreshToken) {
      ...
      const account = users.find((user) => user.id === decoded.sub && user.isActive);
