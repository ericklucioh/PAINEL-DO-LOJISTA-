...
> {
  ...
  resolveByKey: (store: MockStore<TItem>, key: string) => TByKeyItem | undefined;
  createItem: (store: MockStore<TItem>, input: TCreateInput) => TItem;
  toCreateResponse: (item: TItem) => TCreateResponse;
  updateItem: (store: MockStore<TItem>, id: string, input: TUpdateInput) => TItem | undefined;
...
> {
  ...
  return {
    list(query) {
      ...
      const items = store.list().filter((item) => options.filterItem(item, normalizedSearch));
      ...
      const data = items.slice(start, start + options.pageSize).map(options.toListItem);
    ...
    update(id, input) {
      ...
      return updated === undefined ? undefined : options.toUpdateResponse(updated);
    },

    deactivate(id) {
      const deactivated = options.deactivateItem(store, id);
      return deactivated === undefined ? undefined : options.toDeactivateResponse(deactivated);
    },
  };
}
